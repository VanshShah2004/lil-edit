...
const keyForAuth = anonKey ?? serviceRoleKey;

if (!url || !keyForAuth) {
  throw new Error(
    "backend: set SUPABASE_URL and either SUPABASE_ANON_KEY or SUPABASE_SERVICE_ROLE_KEY in backend/.env (or VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY from the frontend)"
  );
}
