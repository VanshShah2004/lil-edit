        ...
        <nav className="flex flex-col flex-1 pt-[3px] pb-2 overflow-y-auto">
          ...
          {!isAdmin && (
            <SideSection label="Your Closet">
              ...
              <SideLink to="/orders" icon={Package} label="Your Orders" pathname={location.pathname} onClick={() => setIsLeftMenuOpen(false)} />
              <SideLink to="/reviews" icon={MessageSquare} label="Your Reviews" pathname={location.pathname} onClick={() => setIsLeftMenuOpen(false)} />
            </SideSection>
          )}
...
// An expandable nav row: a top-level entry (e.g. "Collections") that toggles a
// nested list of sub-links beneath it. Mirrors SideLink's styling but uses a
// button + rotating chevron instead of navigating on click.
