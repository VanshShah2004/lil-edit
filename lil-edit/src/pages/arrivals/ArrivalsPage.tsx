...
// ─── Theme — every colour/copy knob that differs between New Arrivals, Girls
...
export interface ArrivalsTheme {
  ...
  headingGradient: string;           // gradient classes for the accent word
  heroBg: string;                    // hero section gradient
  blobs: [string, string, string];   // blur-blob background colours
  spotlightBg: string;               // SpotlightCard gradient
  spotlightBlob: string;             // text-side blur blob
  accentText: string;                // replaces text-brand-teal accents
  accentHoverText: string;           // hover:text-* on wishlist hearts
  freshBadgeBg: string;              // "Just In" pill background
  tickerWords: string[];
  heroTagline: string;
  emptyTitle: string;
  emptyBlurb: string;
  spotlightLabel: string;
  heroPadY?: string;                 // hero vertical padding override (default "py-28 md:py-32")
}
...
export default function ArrivalsPage({ theme }: { theme: ArrivalsTheme }) {
  const { user } = useAuth();
    ...
    fetchNewArrivals(48, ctrl.signal, theme.gender)
      ...
      .catch((err) => {
        if ((err as Error).name === "AbortError") {
          console.log("[ArrivalsPage] request aborted");
          return;
        }
        console.error("[ArrivalsPage] fetch failed:", err);
        setProducts([]);
      });
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col text-gray-900 overflow-x-hidden">
      ...
      <main className="flex-1 w-full pt-[var(--navbar-height)]">
        {/* HERO */}
        <section className={`relative w-full bg-gradient-to-br ${theme.heroBg} overflow-hidden`}>
          <div className="absolute inset-0 opacity-40 sm:opacity-50">
            <div className={`absolute -top-10 left-1/4 w-72 h-72 ${theme.blobs[0]} rounded-full blur-3xl`}></div>
            <div className={`absolute -bottom-16 right-1/5 w-80 h-80 ${theme.blobs[1]} rounded-full blur-3xl`}></div>
            <div className={`absolute top-1/3 -left-10 w-56 h-56 ${theme.blobs[2]} rounded-full blur-3xl`}></div>
          </div>
          <div className={`relative z-10 flex flex-col items-center text-center px-4 sm:px-6 ${theme.heroPadY ?? "py-28 md:py-32"}`}>
            <span className={`inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-bold uppercase tracking-[0.2em] ${theme.accentText} mb-4`}>
              <Sparkles className="w-3.5 h-3.5" />
              {theme.heroBadge}
            </span>
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-bold leading-none mb-4 text-gray-900">
              {theme.headingLead}<span className={`text-transparent bg-clip-text bg-gradient-to-r ${theme.headingGradient}`}> {theme.headingAccent}</span>
            </h1>
            ...
            >
              {[0, 1].map((copy) => (
                <div key={copy} className="flex items-center" aria-hidden={copy === 1}>
                  {theme.tickerWords.concat(theme.tickerWords).map((w, i) => (
        ...
        <div className="page-container px-4 sm:px-6 pt-5 sm:pt-6 pb-14">
          {loading ? (
            ...
            <div className="w-full py-14 sm:py-20 flex flex-col items-center justify-center text-center bg-white border border-gray-400 rounded-xl px-4 shadow-md">
              ...
              <p className="text-lg sm:text-xl font-semibold text-gray-800 mb-1.5">{theme.emptyTitle}</p>
            ...
            <ArrivalsView products={products} theme={theme} />
          )}
        </div>
      </main>
...
function ArrivalsView({ products, theme }: { products: NewArrivalProduct[]; theme: ArrivalsTheme }) {
  ...
  return (
    <>
      ...
      {spotlightItems.length > 0 && <SpotlightCarousel products={spotlightItems} theme={theme} />}
        ...
        sections.map((section, si) =>
          section.key === "catalog" ? (
            ...
            <section key={section.label} className="mt-10 sm:mt-14">
              ...
              <CollageBento products={section.items} theme={theme} />
            </section>
          ) : (
            <section key={section.label} className={si === 0 && spotlightItems.length === 0 ? "" : "mt-10 sm:mt-14"}>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mb-5">{section.blurb}</p>
              <DropGrid products={section.items} now={now} theme={theme} />
            </section>
          ),
        )
      ) : (
...
function SpotlightCarousel({ products, theme }: { products: NewArrivalProduct[]; theme: ArrivalsTheme }) {
          ...
          {products.map((p) => (
            <CarouselItem key={p.sku} className="pl-0">
              <div className="h-full px-0.5">
                <SpotlightCard product={p} theme={theme} />
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
...
function SpotlightCard({ product: p, theme }: { product: NewArrivalProduct; theme: ArrivalsTheme }) {
  const navigate = useNavigate();
  ...
  return (
    <div
      ...
      className={`group relative h-full rounded-3xl overflow-hidden bg-gradient-to-br ${theme.spotlightBg} text-gray-900 border border-gray-400 cursor-pointer shadow-lg hover:shadow-xl hover:border-gray-500 transition-all`}
    >
      <div className="grid h-full grid-rows-[auto_1fr] md:grid-rows-1 md:grid-cols-[2fr_3fr] md:h-[24rem]">
        ...
        <div className="relative flex flex-col justify-center gap-3 sm:gap-4 p-6 sm:p-10">
          <div className={`absolute top-0 right-0 w-48 h-48 ${theme.spotlightBlob} rounded-full blur-3xl`} />
          <span className={`inline-flex w-fit items-center gap-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-[0.25em] ${theme.accentText}`}>
            <Sparkles className="w-3.5 h-3.5" />
            {theme.spotlightLabel}
          </span>
          ...
          <div className="flex flex-nowrap overflow-hidden items-center gap-3 text-base sm:text-lg text-gray-500 h-8">
            ...
            {p.badges.slice(0, 2).map((b) => (
              <span
                key={b}
                className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-white/90 ${theme.accentText} border border-gray-300 shadow-sm`}
              >
                {b}
              </span>
            ))}
          ...
          <div className="flex flex-col items-start gap-3 md:flex-row md:items-center md:gap-4 mt-1 sm:mt-2">
            <span className="flex items-baseline gap-2 md:mr-auto">
              <span className={`text-2xl sm:text-3xl font-bold ${theme.accentText}`}>₹{p.price}</span>
            ...
            <button
              ...
              className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full border border-gray-400 bg-white shadow-sm flex items-center justify-center transition-colors hover:bg-gray-50 ${
                wishlisted ? "text-primary" : `text-gray-600 ${theme.accentHoverText}`
              }`}
...
function DropGrid({ products, now, theme }: { products: NewArrivalProduct[]; now: number; theme: ArrivalsTheme }) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 [grid-auto-flow:dense]">
      {products.map((p, idx) => (
        <ArrivalCard key={p.sku} product={p} now={now} big={idx % 7 === 3} delayMs={Math.min(idx, 8) * 60} theme={theme} />
      ))}
    </div>
  );
}
...
function CollageBento({ products, theme }: { products: NewArrivalProduct[]; theme: ArrivalsTheme }) {
  const navigate = useNavigate();
  const { addToWishlist, removeFromWishlist, wishlistItems } = useWishlist();
...
function ArrivalCard({
  product: p, now, big, delayMs, theme,
}: {
    ...
    >
      ...
      <div className="absolute top-2.5 left-2.5 right-2.5 flex items-start justify-between">
        <div className="flex flex-col gap-1.5 items-start">
          {fresh && (
            <span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full ${theme.freshBadgeBg} text-white shadow-sm`}>
              Just In
            </span>
          )}
          {p.badges[0] && (
            <span className={`text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-white/90 ${theme.accentText} shadow-sm`}>
              {p.badges[0]}
            </span>
          )}
        </div>
