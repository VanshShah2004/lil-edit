-- Re-key the launch "replace" from slug → base_sku.
--
-- launch_product_atomic (20260520_atomic_product_mutations.sql) removed any existing
-- published + draft rows for the product's SLUG before inserting the fresh published
-- row. That was safe only while slugs were unique. Now that slugs are non-unique
-- (lil-edit migration 20260619_slug_not_unique.sql — two products may share a title →
-- slug), deleting by slug would wipe a DIFFERENT product that merely shares the slug
-- (and cascade-delete its variants/images). base_sku is still globally unique, so it
-- is the correct identity for "replace this product's rows".
--
-- Only the two DELETE predicates change; the insert logic is identical to 20260520.
